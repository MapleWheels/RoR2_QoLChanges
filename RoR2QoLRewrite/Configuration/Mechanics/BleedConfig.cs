...
namespace RoR2QoLRewrite.Configuration.Mechanics
{
    public class BleedConfig : ConfigDataModel
    {
        ...
        public static ConfigData<float> Bleed_BaseDamageRatio { get; set; } = new ConfigData<float>()
        {
            DescriptionString = "The bleed damage scale multiplier. Scales off of Base Damage",
            DefaultValue = 1.8f
        };

        public static ConfigData<float> Dagger_ProcChance { get; set; } = new ConfigData<float>()
        {
            DescriptionString = "Item: Tri-Tip Dagger's Proc chance per item.",
            DefaultValue = 7f
        };

        public static ConfigData<float> StandardBleed_TimeSecs { get; set; } = new ConfigData<float>()
        {
            DescriptionString = "The standard bleed damage debuff time.",
            DefaultValue = 4f
        };

        public static ConfigData<float> ShatterspleenBleed_TimeSecs { get; set; } = new ConfigData<float>()
        {
        ...
        {
            SectionName = "Mechanics_Bleeding";
        }
    }
}
